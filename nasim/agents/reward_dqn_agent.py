...
class DQNAgent:
    ...
    def run_train_episode(self, step_limit):
        ...
        while not done and not env_step_limit_reached and steps < step_limit:
            ...
            next_o, raw_r, done, env_step_limit_reached, _ = self.env.step(a)
            ...
            # 2. 信息增益奖励（观测发生变化）
            if not np.array_equal(next_o, last_obs):
                shaped_r += info_gain_bonus

            # 3. 成功奖励（达到目标）
            if self.env.goal_reached():
                shaped_r += success_bonus

            # 4. 使用原始 r（例如漏洞利用成功的奖励等原生奖励）
            shaped_r += raw_r

            # 更新上一步观测
            last_obs = np.array(next_o, copy=True)

            # 存入buffer（使用强化后的 reward）
            self.replay.store(o, a, next_o, shaped_r, done)
