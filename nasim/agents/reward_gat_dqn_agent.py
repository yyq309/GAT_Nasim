...
class GATDQNAgent:
    ...
    def run_train_episode(self, step_limit):
        ...
        while not done and steps < step_limit:
            ...
            # ---------------------------------------------------------
            # Encode obs & next_obs into graph vectors
            # ---------------------------------------------------------
            num_nodes = len(self.env.network.hosts) + 1

            if obs_array.ndim == 1:
                feature_dim = obs_array.size // num_nodes
                obs_array = obs_array.reshape(num_nodes, feature_dim)
            if next_obs_array.ndim == 1:
                feature_dim = next_obs_array.size // num_nodes
                next_obs_array = next_obs_array.reshape(num_nodes, feature_dim)

            graph_s = obs_to_graph(obs_array, self.env.network).to(self.device)
            graph_ns = obs_to_graph(next_obs_array, self.env.network).to(self.device)

            s_vec = self.encoder(graph_s).detach().cpu().numpy()
            next_vec = self.encoder(graph_ns).detach().cpu().numpy()
            # ---------------------------------------------------------
...
if __name__ == "__main__":
    ...
    env = nasim.make_benchmark(args.env_name, seed=args.seed, fully_obs=False, flat_actions=True, flat_obs=True)
